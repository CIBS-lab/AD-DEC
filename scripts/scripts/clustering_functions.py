...
def dec_vae_cluster(encoder, feature_array, y, n_classes, seed=5192,
    ...
    print(f"Running X-DEC clustering with n_classes={n_classes}")
    ...
    y_pred = kmeans.fit_predict(encoder.predict(feature_array[0], feature_array[1]))
    ...
    for ite in range(int(maxiter)):
        if ite % update_interval == 0:
            print(f"Iteration {ite}: Generating soft labels...")
            # Generate soft labels for the samples
            q = model.predict(feature_array, verbose=0)
            print(f"Iteration {ite}: Soft labels generated.")
            # update the auxiliary target distribution p
            p = target_distribution(q)
        ...
        print(f"Iteration {ite}: Performing SGD optimisation...")
        # Perform one iteration of the SGD optimisation on the sample subset.
        loss = model.train_on_batch(x=[feature_array[0][idx, :],
                                       feature_array[1][idx, :]], y=p[idx])
        print(f"Iteration {ite}: SGD optimisation completed.")

        # Free up memory after each SGD optimization step
        #tf.keras.backend.clear_session()
        #gc.collect()
        ...
        print(f"Iteration {ite}: Current memory usage: {current / 10**6:.2f} MB; Peak: {peak / 10**6:.2f} MB")
    ...
    print("X-DEC clustering completed.")
...
def cluster_mlp_autoencoder(feature_array, n_classes, y=None, neurons_h=64,
    ...
    print("DEC model training completed.")
...
def cluster_mlp_vae(feature_array, n_classes, y=None, ds1=52, ds2=5,
                    ...
                    weighted=True, check_stability=False, stability_check_it=100,
    ...
    stability_check_it : int, optional
        The number of iterations to run for determining stability. The default is 100.
    ...
    verbose : int, optional
        Integer defining the level of verbosity, with 0 being the least
        verbose. The default is 0.
    ...
    encoder_unoptimised : Keras object,
        ...
        optimisation, meaning in its initial form, only trained to best
        reproduce the input variables. Only returned if return_extra=True.

    '''
    print("Training X-DEC model...")
    ...
    # Define cluster numbers based on number of samples per cluster
    if order_by_size:
        print("Ordering clusters by size...")
        y_pred = pd.Series(y_pred)
        y_pred.replace(np.asarray(y_pred.value_counts().index),
                       np.arange(1, n_classes + 1),
                       inplace=True)
    ...
    # Get latent feature space Z
    print("Getting latent feature space...")
    Z = autoencoder.predict(feature_array[0], feature_array[1])
    print("Latent feature space obtained.")
    
    print("X-DEC model training completed.")
...
def compute_cluster_stability(feature_array, n_cluster, k=10, rep=5,
                              ...
                              majority_vote=False, disable_resampling=False,
    ...
    ds1 : int, optional
        Number of neurons in the first hidden layer of input set 1 for VAE models. 
        Only used if model_type="vae". The default is 52.
    ds2 : int, optional
        Number of neurons in the first hidden layer of input set 2 for VAE models. 
        Only used if model_type="vae". The default is 5.
    ds12 : int, optional
        Number of neurons in the hidden layer that joins input set 1 and 2 for VAE models. 
        Only used if model_type="vae". The default is 48.
    ls : int, optional
        Number of neurons in the embedding layer, also known as the bottleneck layer, 
        which stores the latent features. Only used if model_type="vae". The default is 32.
    act : str, optional
        Activation function to use in the hidden layers. Only used if model_type="vae". 
        The default is "elu".
    dropout : float, optional
        Dropout rate. Only used if model_type="vae". The default is 0.2.
    distance : str, optional
        Distance metric to use for regularization in the objective function. 
        Only used if model_type="vae". The default is "mmd".
    beta : float, optional
        Influence of the disentanglement factor. Only used if model_type="vae". 
        The default is 25.
    mapper : str, optional
        Method to map clusters across iterations. Options are "centroids", "overlap", and "jaccard". 
        The default is "centroids".
    retrain_autoencoder : bool, optional
        Whether to completely retrain the autoencoder (or X-VAE) in each iteration. 
        The default is True.
    weighted : bool, optional
        Whether to scale the influence of input sets 1 and 2 according to the number of values they contain. 
        The default is True.
    majority_vote : bool, optional
        Whether to use a majority vote for the reference cluster for stability assessment. 
        If False, the first cluster assignment is used as the reference. The default is False.
    disable_resampling : bool, optional
        If True, 'k' and 'rep' are ignored, and all samples are used during each iteration to determine 
        baseline model stability. The total number of iterations will still be based on the product of 'k' and 'rep'. 
        The default is False.
    seed : int, optional
        Random seed. The default is 5192.
    return_extra : bool, optional
        Whether to return additional variables (Z, cluster_labels, centroids, and y_pred). 
        The default is False.
    model_type : str, optional
        Type of clustering model to run ("autoencoder" or "vae"). The default is "autoencoder".
    save : bool, optional
        Whether to save results locally. The default is True.
    save_name : str, optional
        Name under which to save the results if save=True. The default is "stability_check".

    Returns
    -------
    cluster_stab : pandas.DataFrame
        DataFrame containing the cluster-wise stabilities.
    Z : pandas.DataFrame, optional
        DataFrame containing the latent features from the clustering model trained on the full dataset. 
        Returned if return_extra=True.
    cluster_labels : ndarray, optional
        Cluster labels for each sample. Returned if return_extra=True.
    centroids : ndarray, optional
        Centroids of the clusters in the Z-space. Returned if return_extra=True.
    y_pred : pandas.DataFrame, optional
        DataFrame containing the cluster memberships for all samples for each iteration/subset. 
        Returned if return_extra=True.
    model : Keras object
        The clustering model.
    encoder : Keras object, optional
        The encoder of the clustering model. Returned if return_extra=True.

    Notes
    -----
    When mapper="jaccard", cluster_stab holds the per-cluster Jaccard
    coefficients. Summarise reproducibility as the mean across clusters (the
    whole-model value, 0.54 for the six-phenotype WRAP solution), and pair it
    with the sample-wise stability (73%). Do not headline a single stable
    cluster's value as the reproducibility of the whole solution.
    '''
    
    # Convert DataFrame to Numpy array if necessary
    if isinstance(feature_array, list) and isinstance(feature_array[0], pd.DataFrame):
        warnings.warn("Data was supplied as DataFrame, converting it to a Numpy array.")
        feature_array[0] = np.array(feature_array[0])
        feature_array[1] = np.array(feature_array[1])
    elif isinstance(feature_array, pd.DataFrame):
        warnings.warn("Data was supplied as DataFrame, converting it to a Numpy array.")
        feature_array = np.array(feature_array)
        
    print(f"Computing cluster stability with n_cluster={n_cluster}, k={k}, rep={rep}")
    ...
    if model_type == "autoencoder":
        ...
        y_pred = pd.DataFrame(index=range(x1.shape[0]),
                              columns=range(k*rep))
        if not retrain_autoencoder:
            encoder_unoptimised.save_model("temp_xvae_model_copy")
            print('Saved temp_vae_model_copy')
    ...
    for train_index, test_index in rkf.split(feature_array):
        if disable_resampling:
            train_index = np.arange(len(feature_array))
            test_index = np.arange(len(feature_array))
        ...
        if mapper == "centroids":
            ...
            raise ValueError(f"'{mapper}' mapper not recognised! Must be 'centroids', 'overlap', or 'jaccard'.")
        ...
        print('y_pred length ' + str(len(y_pred)))

        # Print the size of all local variables
        local_vars = locals()
        for var_name, var_value in local_vars.items():
            size_in_bytes = sys.getsizeof(var_value)
            size_in_mb = size_in_bytes / (1024 ** 2)  # Convert from bytes to MB
            print(f"Variable '{var_name}' size: {size_in_bytes} bytes ({size_in_mb:.6f} MB)")
    ...
    if mapper == "jaccard":
        ...
        if majority_vote:
            y_pred, cluster_stab = jaccard_similarity(cluster_labels, y_pred, True)
    else:
    ...
    print("Cluster stability computation completed.")
...
def sample_cluster_stability(majority_vote, y_pred,
    ...
    *args : Pandas.Series
    ...
    Notes
    -----
    The mean of pred_stability is the sample-wise stability reported in the
    paper (73%, median 74%), following de Kok et al. 2024. It is reported
    alongside the mean cluster-wise Jaccard (0.54; see jaccard_similarity), not
    in place of it.
...
def jaccard_similarity(x, x_pred, majority_vote = False):
    ...
    jac_mat_final : pandas.DataFrame
        Maximum Jaccard coefficient per reference cluster (rows) per resampling
        iteration (columns).

    Notes
    -----
    Report overall stability as the mean of jac_mat_final across clusters and
    iterations; this is the whole-model number (0.54 for the six-phenotype WRAP
    solution, following de Kok et al. 2024 and Castela Forte et al. 2021). A
    single highly stable cluster (the younger-tau phenotype, per-cluster Jaccard
    near 0.99) must not be reported as the reproducibility of the whole solution.
    Sample-wise stability (sample_cluster_stability) is the complementary
    summary, 73% in the paper.
    '''
