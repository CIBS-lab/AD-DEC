...
sys.path.insert(1, '/groups/rbrinton/coco/wrap/scripts')
...
from common import sse, bce, mmd, sampling, kl_regu
...
class xvae():
    ...
    def build_model(self):
        ...
        if self.args.distance == "mmd":
            true_samples = tf.random.normal((self.args.bs, self.args.ls))
        ...
        # Update the optimizer instantiation to comply with legacy TensorFlow 2.15 style
        adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999, decay=0.0)
        
        self.vae.compile(optimizer=adam, metrics=[binary_crossentropy])
